...
DWORD WINAPI BlurThread(const LPVOID lpParams)
{
	...
	std::cout << std::to_string(info->threadNum) + " " + std::to_string((clock() - info->start) / (float)CLOCKS_PER_SEC) + "\n";
	ExitThread(0);
}

void BlurInThreads(int n, std::vector<std::vector<Pixel>>& pixels, int i)
{
...
void BlurImage(int n, const std::string inFileName, const std::string& outFileName, int i)
{
